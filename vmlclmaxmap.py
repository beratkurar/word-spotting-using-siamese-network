# -*- coding: utf-8 -*-
"""
Created on Thu Jun 22 13:46:04 2017

@author: B
Architecture from Jadeberg et al. 2016
Contrastive loss from Hadsell et al. 2006
train on vml
test on vml
equal pairs
train until max map
mAP with query itself
mAP without query itself
"""
...
(x_train, y_train)=loaddata(book+'segr20',100,3)
...
(x_val, y_val)=loaddata(book+'segr10',20,10)
...
#mcp=ModelCheckpoint( filepath='s1bestmodelf'+str(fold)+'.h5',monitor='val_loss', save_best_only=True, save_weights_only=False,verbose=1)
...
del model
model=load_model('smap3bestmodel.h5',custom_objects={"contrastive_loss":contrastive_loss})

print('start testing WITH query itself')

(x_test, y_test)=loaddata(book+'segr100',21,100)
